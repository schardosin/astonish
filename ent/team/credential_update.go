...
func (_u *CredentialUpdate) check() error {
	if v, ok := _u.mutation.Name(); ok {
		if err := credential.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`team: validator failed for field "Credential.name": %w`, err)}
		}
	}
	if v, ok := _u.mutation.CredType(); ok {
		if err := credential.CredTypeValidator(v); err != nil {
			return &ValidationError{Name: "cred_type", err: fmt.Errorf(`team: validator failed for field "Credential.cred_type": %w`, err)}
		}
	}
	return nil
}
...
func (_u *CredentialUpdateOne) sqlSave(ctx context.Context) (_node *Credential, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`team: missing "Credential.id" for update`)}
	}
