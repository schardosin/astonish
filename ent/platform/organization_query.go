...
type OrganizationQuery struct {
	...
	withUsers         *UserQuery
	withOidcProviders *OIDCProviderQuery
...
// QueryOidcProviders chains the current query on the "oidc_providers" edge.
func (_q *OrganizationQuery) QueryOidcProviders() *OIDCProviderQuery {
...
func (_q *OrganizationQuery) QueryLoginSessions() *LoginSessionQuery {
	query := (&LoginSessionClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withOidcProviders: _q.withOidcProviders.Clone(),
...
// WithOidcProviders tells the query-builder to eager-load the nodes that are connected to
// the "oidc_providers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithOidcProviders(opts ...func(*OIDCProviderQuery)) *OrganizationQuery {
	query := (&OIDCProviderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOidcProviders = query
	return _q
}
...
func (_q *OrganizationQuery) WithLoginSessions(opts ...func(*LoginSessionQuery)) *OrganizationQuery {
	query := (&LoginSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withOidcProviders; query != nil {
		if err := _q.loadOidcProviders(ctx, query, nodes,
			func(n *Organization) { n.Edges.OidcProviders = []*OIDCProvider{} },
			func(n *Organization, e *OIDCProvider) { n.Edges.OidcProviders = append(n.Edges.OidcProviders, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withLoginSessions; query != nil {
		if err := _q.loadLoginSessions(ctx, query, nodes,
			func(n *Organization) { n.Edges.LoginSessions = []*LoginSession{} },
			func(n *Organization, e *LoginSession) { n.Edges.LoginSessions = append(n.Edges.LoginSessions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrganizationQuery) loadOidcProviders(ctx context.Context, query *OIDCProviderQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OIDCProvider)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(oidcprovider.FieldOrgID)
	}
	query.Where(predicate.OIDCProvider(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.OidcProvidersColumn), fks...))
	}))
...
func (_q *OrganizationQuery) loadLoginSessions(ctx context.Context, query *LoginSessionQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *LoginSession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(loginsession.FieldOrgID)
	}
	query.Where(predicate.LoginSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.LoginSessionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
