...
import {
  ...
  fetchFleetThreads, fetchFleetMessages,
} from '../api/fleetChat'
import type {
  FleetDefinition, FleetPlanSummary, FleetSession, FleetPlanStatus,
  FleetTrace, FleetThread, FleetMessage,
} from '../api/fleetChat'
...
interface FleetChatSession extends ChatSession {
  fleetKey?: string
  fleetName?: string
...
interface FleetPlanStatusExt extends FleetPlanStatus {
...
interface FleetThreadExt extends FleetThread {
  agent_key?: string
}

/** Structured plan data (fetchFleetPlan returns Record<string, unknown>). */
interface FleetPlanData {
  name?: string
  description?: string
  created_from?: string
  channel?: {
...
function extractAgentRole(sessionLabel: string): string | null {
...
function PlanDetail({ planKey, onNavigate, onRefresh, theme }: PlanDetailProps) {
...
function SessionTrace({ sessionId, onRefresh, onNavigate }: SessionTraceProps) {
  ...
  const [liveSession, setLiveSession] = useState<FleetSession | null>(null)
...
function TemplateDetail({ templateKey, templates, onCreatePlan }: TemplateDetailProps) {
