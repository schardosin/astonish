...
func (_q *AppQuery) QueryState() *AppStateQuery {
	query := (&AppStateClient{config: _q.config}).Query()
...
func (_q *AppQuery) WithState(opts ...func(*AppStateQuery)) *AppQuery {
	query := (&AppStateClient{config: _q.config}).Query()
...
func (_q *AppQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*App, error) {
	...
	if query := _q.withState; query != nil {
		if err := _q.loadState(ctx, query, nodes,
			func(n *App) { n.Edges.State = []*AppState{} },
			func(n *App, e *AppState) { n.Edges.State = append(n.Edges.State, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AppQuery) loadState(ctx context.Context, query *AppStateQuery, nodes []*App, init func(*App), assign func(*App, *AppState)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(appstate.FieldAppID)
	}
	query.Where(predicate.AppState(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(app.StateColumn), fks...))
	}))
