...
func (_q *UserQuery) QueryLoginSessions() *LoginSessionQuery {
...
func (_q *UserQuery) QueryChannels() *UserChannelQuery {
	query := (&UserChannelClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) WithLoginSessions(opts ...func(*LoginSessionQuery)) *UserQuery {
	query := (&LoginSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithChannels(opts ...func(*UserChannelQuery)) *UserQuery {
	query := (&UserChannelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withChannels; query != nil {
		if err := _q.loadChannels(ctx, query, nodes,
			func(n *User) { n.Edges.Channels = []*UserChannel{} },
			func(n *User, e *UserChannel) { n.Edges.Channels = append(n.Edges.Channels, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadLoginSessions(ctx context.Context, query *LoginSessionQuery, nodes []*User, init func(*User), assign func(*User, *LoginSession)) error {
...
func (_q *UserQuery) loadChannels(ctx context.Context, query *UserChannelQuery, nodes []*User, init func(*User), assign func(*User, *UserChannel)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userchannel.FieldUserID)
	}
	query.Where(predicate.UserChannel(func(s *sql.Selector) {
