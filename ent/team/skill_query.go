...
func (_q *SkillQuery) QueryFiles() *SkillFileQuery {
	query := (&SkillFileClient{config: _q.config}).Query()
...
func (_q *SkillQuery) WithFiles(opts ...func(*SkillFileQuery)) *SkillQuery {
	query := (&SkillFileClient{config: _q.config}).Query()
...
func (_q *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := _q.withFiles; query != nil {
		if err := _q.loadFiles(ctx, query, nodes,
			func(n *Skill) { n.Edges.Files = []*SkillFile{} },
			func(n *Skill, e *SkillFile) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SkillQuery) loadFiles(ctx context.Context, query *SkillFileQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *SkillFile)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(skillfile.FieldSkillID)
	}
	query.Where(predicate.SkillFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(skill.FilesColumn), fks...))
	}))
