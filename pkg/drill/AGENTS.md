...
- **Tutorial vs test**: `drill_config.mode: tutorial` enables narration/hold/record; **content assertions fail the run** (no soft-pass on empty/broken UI). Authoring is explore-first then blueprint (Scene|Voiceover|Visual); only screen rows record MP4s. `validate_drill` rejects TODO stubs, navigate-only recorded scenes, and missing content asserts. Tutorial drills belong in **dedicated tutorial suites**. `/tutorial-drill-add` (alias `/tutorial-add`) is gated by `IsTutorialSuite`. Do **not** mix tutorial tags into default fleet smoke without filtering `mode!=tutorial`.
- **Human capture**: `browser_request_human(capture_actions:true)` + `draft_drill_from_action_log` yields a **mode-neutral** step skeleton. Chat specializes it (asserts for `/drill` tests, or tutorial fields after blueprint). Do not put `mode` / `record` / narration inside the draft tool.
