...
export default function SkillsSettings({ config, onSaved, theme = 'dark' }) {
  // Config form state
  ...
  // Editor state
  const [activeSkill, setActiveSkill] = useState(null) // { name, content, rawFile, editable, source }
  const [editorContent, setEditorContent] = useState('')
  const [editorMode, setEditorMode] = useState(null) // 'view' or 'edit'
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      await saveFullConfigSection('skills', saveData)
      setSaveSuccess(true)
      if (onSaved) onSaved()
  ...
  const handleOpenSkill = async (name, mode) => {
      ...
      <div className="flex flex-col h-full" style={{ margin: '-24px', height: 'calc(100% + 48px)' }}>
        ...
        <div className="flex-1 overflow-hidden">
          {editorLoading ? (
            ...
            <CodeMirror
              value={editorContent}
              onChange={editorMode === 'edit' ? (value) => setEditorContent(value) : undefined}
...
// Skill row component
function SkillRow({ skill, onView, onEdit, onDelete }) {
  return (
    <div
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {skill.require_bins?.length > 0 && !skill.eligible && (
