...
import type { FullConfig } from './settings/settingsApi'
...
interface WebCapableTools {
  webSearch: WebTool[]
  webExtract: WebTool[]
}

interface StandardServerEnvVar {
  name: string
  required: boolean
}

interface StandardServer {
  id: string
  displayName: string
  isDefault: boolean
  installed: boolean
...
const saveSettings = async (data: Record<string, unknown>): Promise<unknown> => {
  const res = await fetch('/api/settings/config', {
    method: 'PUT',
...
const replaceAllProviders = async (providers: Record<string, unknown>[]): Promise<unknown> => {
  const res = await fetch('/api/settings/config', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
const fetchMCPConfig = async (): Promise<MCPConfigData> => {
  const res = await fetch('/api/settings/mcp')
  if (!res.ok) throw new Error('Failed to fetch MCP config')
  return res.json()
}

const saveMCPConfig = async (data: Record<string, unknown>): Promise<unknown> => {
  const res = await fetch('/api/settings/mcp', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data)
...
const fetchWebCapableTools = async (): Promise<WebCapableTools> => {
...
// Taps API functions
const fetchTaps = async (): Promise<{ taps: TapEntry[] }> => {
  const res = await fetch('/api/taps')
  if (!res.ok) throw new Error('Failed to fetch taps')
  return res.json()
}

const addTap = async (url: string, alias: string = ''): Promise<unknown> => {
  const res = await fetch('/api/taps', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
const removeTap = async (name: string): Promise<unknown> => {
...
const fetchMCPStatus = async (): Promise<{ servers: MCPServerStatusEntry[] }> => {
...
const toggleMCPServer = async (name: string, enabled: boolean): Promise<unknown> => {
...
const refreshMCPServer = async (serverName: string): Promise<unknown> => {
...
export default function SettingsPage({ onClose, activeSection = 'general', onSectionChange, onToolsRefresh, onSettingsSaved, updateAvailable = null, onUpdateClick = null, appVersion = 'dev', theme = 'dark' }: SettingsPageProps) {
  ...
  const [mcpViewMode, setMcpViewMode] = useState<'editor' | 'source'>('editor') // 'editor' or 'source'
  ...
  const [setupServer, setSetupServer] = useState<string | null>(null) // Server being set up
  const [setupEnv, setSetupEnv] = useState<Record<string, string>>({}) // Env vars being entered
  ...
  const [tapsSuccess, setTapsSuccess] = useState<string | null>(null) // Success message for refresh
