-- Platform schema (SQLite) — desired state.
...
--
-- NOTE: Uses explicit CREATE UNIQUE INDEX instead of inline UNIQUE constraints
-- to match Atlas's normalized representation and avoid cosmetic no-op diffs.
...
CREATE UNIQUE INDEX IF NOT EXISTS users_oidc_issuer_oidc_subject ON users(oidc_issuer, oidc_subject);
...
CREATE UNIQUE INDEX IF NOT EXISTS oidc_providers_org_id_issuer_url ON oidc_providers(org_id, issuer_url);
...
CREATE UNIQUE INDEX IF NOT EXISTS user_channels_channel_type_external_id ON user_channels(channel_type, external_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS platform_mcp_servers_name ON platform_mcp_servers(name);
...
CREATE UNIQUE INDEX IF NOT EXISTS sandbox_templates_scope_owner_id_slug ON sandbox_templates(scope, owner_id, slug);
