...
- **Runtime readiness**: `run_drill` injects suite `credentials`/`credential_injection` (or fleet-plan fallback) before the runner executes `configure:` then `setup:` then `ready_check`. Do not put secrets in `bootstrap_files`.
