...
import type { DrillSuiteSummary, DrillSuiteDetail, DrillDetail } from '../api/drillApi'
...
function statusColor(status: string): StatusColors {
  switch (status) {
...
function DrillSidebar({ suites, selectedItem, onSelect, onSearch, searchQuery, isLoading }: DrillSidebarProps) {
...
function SuiteDetail({ suiteKey, onNavigate, onRunSuite, onAddDrills, onRefresh, theme }: SuiteDetailProps) {
...
function DrillDetailView({ suiteKey, drillKey, onNavigate, theme }: DrillDetailProps) {
...
function StepCard({ node, index }: StepCardProps) {
...
function ReportStepCard({ step, index }: ReportStepCardProps) {
  const [expanded, setExpanded] = useState(false)
