...
const (
	...
	DockerImageRepo = "astonish/incus"
...
func pullOrFallback(imageTag string) error {
	...
	if output, err := pullCmd.CombinedOutput(); err != nil {
		...
		return fmt.Errorf("Docker image %s not found (pull failed: %s).\n"+
			...
			"  make docker-incus\n"+
			"  docker tag astonish/incus:dev astonish/incus:latest",
...
// time inside the Docker container.
func WaitForIncusAPI(timeout time.Duration) error {
	deadline := time.Now().Add(timeout)

	for time.Now().Before(deadline) {
		// Try connecting via the Incus Go SDK (same path as Connect())
	...
	return fmt.Errorf("timeout waiting for Incus API on %s (waited %s)", DockerContainerName, timeout)
}
