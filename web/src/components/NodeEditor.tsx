...
type NodeType = 'input' | 'llm' | 'tool' | 'updateState' | 'update_state' | 'output'
...
function VariablePanel({ variableGroups, activeTextareaRef, getValue, setValue, onVariableClick }: VariablePanelProps) {
...
const HighlightedTextarea = React.forwardRef<HTMLTextAreaElement, HighlightedTextareaProps>(function HighlightedTextarea(
 ...
 */
interface RawToolOutputEditorProps {
  value: Record<string, string> | undefined
  onChange: (val: Record<string, string> | undefined) => void
}

function RawToolOutputEditor({ value, onChange }: RawToolOutputEditorProps) {
 ...
 */
interface OutputModelEditorProps {
  value: Record<string, string> | undefined
...
function OutputModelEditor({ value, onChange, theme, hideLabel = false, singleField = false }: OutputModelEditorProps) {
...
function UpdateStateForm({ data, onChange, theme }: UpdateStateFormProps) {
...
function InputNodeForm({ data, onChange, theme }: InputNodeFormProps) {
...
function LlmNodeForm({ data, onChange, theme, availableTools = [], availableVariables = [] }: LlmNodeFormProps) {
...
function ToolNodeForm({ data, onChange, theme, availableTools = [] }: ToolNodeFormProps) {
  const currentTools: string[] = data.tools_selection || []
  const toolNames = availableTools.map(t => t.name)
...
function OutputNodeForm({ data, onChange, theme, availableVariables = [] }: OutputNodeFormProps) {
